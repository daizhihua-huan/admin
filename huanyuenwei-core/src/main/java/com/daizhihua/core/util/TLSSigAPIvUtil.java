...
public class TLSSigAPIvUtil {
    final private long sdkappid;
    final private String key;

    public TLSSigAPIvUtil(long sdkappid, String key) {
    ...
    private String genUserSig(String userid, long expire, byte[] userbuf) throws JSONException {
        ...
        sigDoc.append("TLS.ver", "2.0");
        sigDoc.append("TLS.identifier", userid);
        sigDoc.append("TLS.sdkappid", sdkappid);
        sigDoc.append("TLS.expire", expire);
        sigDoc.append("TLS.time", currTime);

        String base64UserBuf = null;
        if (null != userbuf) {
            base64UserBuf = Base64.getEncoder().encodeToString(userbuf).replaceAll("\\s*", "");
            sigDoc.append("TLS.userbuf", base64UserBuf);
        }
        String sig = hmacsha256(userid, currTime, expire, base64UserBuf);
        if (sig.length() == 0) {
            return "";
        }
        sigDoc.append("TLS.sig", sig);
