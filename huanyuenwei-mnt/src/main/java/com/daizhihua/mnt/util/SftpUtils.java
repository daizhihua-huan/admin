...
import com.daizhihua.mnt.minotor.FileProgressMonitor;
import com.daizhihua.mnt.minotor.SftpMonitor;
...
public final class SftpUtils {
	...
	public SftpUtils(WebShellData sshData) {
		...
		this.password = sshData.getPassword();
	 ...
	 * @date 2021/3/2 23:36
	 */
	public void upload(String directory, String sftpFileName, InputStream input,Long length,Long userId) throws SftpException {
		long start = System.currentTimeMillis();
		// 创建不存在的文件夹，并切换到文件夹
		createDir(directory);
		// 上传文件
		channelSftp.put(input, sftpFileName,new SftpMonitor(length,userId),ChannelSftp.OVERWRITE);
	...
	public InputStream download(String path) {
		// 文件所在目录
		String directory = path.substring(0, path.lastIndexOf(Constant.SEPARATOR));
		// 文件名
		String fileName = path.substring(path.lastIndexOf(Constant.SEPARATOR) + 1);
	...
	private boolean delete(String directory, String fileName) {
		String file = directory + Constant.SEPARATOR + fileName;
		try {
			ChannelSftp.LsEntry lsEntry = (ChannelSftp.LsEntry) listFiles(file).get(0);
			// 用户权限处理
			if (!(Constant.USER_ROOT.equals(username)
		...
		if (isFileExists(path) || vector.isEmpty()) {
			...
			vector.forEach(v -> {
				...
				if (FileType.DIRECTORY.getSign().equals(lsEntry.getLongname().substring(0, 1))) {
					delFlag.set(delete(path + Constant.SEPARATOR + lsEntry.getFilename()));
				} else {
					// 删除文件
		...
		try {
			if (isDirExists(directory) || isFileExists(directory)) {
				...
				while (it.hasNext()) {
					...
					if (Constant.DOT.equals(lsEntry.getFilename())
							|| Constant.PARENT_DIRECTORY.equals(lsEntry.getFilename())) {
						it.remove();
					}
				}
				return vector;
	...
	public boolean createDir(String createPath) {
		try {
			...
			String[] pathArray = createPath.split(Constant.SEPARATOR);
			StringBuilder filePath = new StringBuilder(Constant.SEPARATOR);
			for (String path : pathArray) {
				...
				filePath.append(Constant.SEPARATOR);
